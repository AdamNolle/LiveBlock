#!/usr/bin/env python3
"""Verify one offline ONNX release/update bundle against its embedded public ring."""
...
def verify(bundle: Path) -> dict:
    ...
    if artifact.is_symlink() or not artifact.is_file():
        raise ValueError("bundle requires a regular ONNX artifact")
    ...
    if artifact_sha256(artifact) != manifest["artifactSha256"]:
        raise ValueError("ONNX artifact fingerprint mismatch")
    ...
    try:
        ...
        Ed25519PublicKey.from_public_bytes(public_key).verify(
            signature, _signing_bytes(manifest)
        )
...
def main() -> int:
    ...
    print(
        f"verified signed ONNX bundle: model={manifest['modelId']} "
